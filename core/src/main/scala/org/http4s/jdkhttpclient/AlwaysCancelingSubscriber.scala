...
/** An implementation of a JRE `Flow.Subscriber` which immediately cancels the `Flow.Subscription`.
  *
  * We use this to clean up resources and conform the JRE API contract in cases where the HTTP body
  * is not read.
  */
